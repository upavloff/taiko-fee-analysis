...
class Individual {
    constructor(alpha_data = 0.22, nu = 0.2, H = 72) {
        this.alpha_data = alpha_data;  // L1 DA gas ratio [0.15, 0.30]
        this.nu = nu;                  // Deficit weight parameter [0.02, 1.0]
        this.H = H;                    // Horizon parameter (24-step aligned)
     ...
     */
    clone() {
        const copy = new Individual(this.alpha_data, this.nu, this.H);
     ...
     */
    getParameters() {
        return [this.alpha_data, this.nu, this.H];
    }
...
class ParameterBounds {
    constructor() {
        this.alphaBounds = [0.15, 0.30];  // Alpha-data empirical range
        this.nuBounds = [0.02, 1.0];
        this.HBounds = [24, 288];         // Updated range for alpha model

        // Pre-computed 24-step aligned H values for efficiency
        this.validHValues = [];
        for (let h = this.HBounds[0]; h <= this.HBounds[1]; h += 24) {
    ...
    generateRandomIndividual() {
        const alpha_data = Math.random() * (this.alphaBounds[1] - this.alphaBounds[0]) + this.alphaBounds[0];
        ...
        return new Individual(alpha_data, nu, H);
    }
    ...
    repairIndividual(individual) {
        // Clamp alpha_data and nu to bounds
        individual.alpha_data = Math.max(this.alphaBounds[0], Math.min(this.alphaBounds[1], individual.alpha_data));
...
class GeneticOperators {
    ...
    crossover(parent1, parent2) {
        ...
        for (let i = 0; i < 2; i++) { // Only crossover alpha_data and nu (not H)
            if (Math.random() <= 0.5) {
                const y1 = params1[i];
                const y2 = params2[i];

                if (Math.abs(y1 - y2) > 1e-14) {
                    const lb = i === 0 ? this.bounds.alphaBounds[0] : this.bounds.nuBounds[0];
                    const ub = i === 0 ? this.bounds.alphaBounds[1] : this.bounds.nuBounds[1];
...
class TaikoFeeEvaluator {
    ...
    async evaluate(individual) {
        ...
        try {
            ...
            console.warn(`Evaluation failed for (α=${individual.alpha_data}, ν=${individual.nu}, H=${individual.H}):`, error);
