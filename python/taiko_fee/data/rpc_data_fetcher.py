...
class ImprovedRealDataIntegrator:
    ...
    def __init__(self, cache_dir: Optional[str] = None):
        if cache_dir is None:
            # Default to project root data_cache directory
    ...
    def get_real_basefee_data(self, start_date: str, end_date: str,
        ...
        cache_file = self.cache_dir / f"basefee_{start_date}_{end_date}_{provider}.csv"

        # Try cache first
        if use_cache and cache_file.exists():
            try:
                df = pd.read_csv(cache_file, parse_dates=['timestamp'])
                print(f"✓ Loaded {len(df)} records from cache: {cache_file.name}")
                return df
            except Exception as e:
                print(f"⚠ Cache read error: {e}, fetching fresh data")

        # Fetch from RPC
        print(f"⏳ Fetching fresh data from {provider}...")
        ...
        try:
            ...
            print(f"✓ Cached {len(df)} records to {cache_file.name}")
        except Exception as e:
