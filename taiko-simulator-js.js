...
class SimulationParams {
    constructor(options = {}) {
        ...
        this.gas_per_batch = options.gas_per_batch || 21000 * this.txs_per_batch;
...
class ResearchTaikoFeeSimulator {
    ...
    estimateL1CostPerTx(l1BasefeeGwei) {
        ...
        const gasCostPerTx = this.params.gas_per_batch / this.params.txs_per_batch;
        const costPerTx = l1BasefeeWei * gasCostPerTx / 1e18;  // Convert to ETH
    ...
    calculateEstimatedFee(l1CostEstimate) {
        ...
        // Enhanced deficit component with horizon prediction
        let deficitComponent = 0;
        if (deficit > 0) {
            const deficitRatio = deficit / target;
            deficitComponent = this.params.nu * deficitRatio * l1CostEstimate;
        }
...
class ImprovedResearchTaikoFeeSimulator extends ResearchTaikoFeeSimulator {
    ...
    estimateL1CostPerTx(l1BasefeeGwei) {
        ...
        const gasCostPerTx = this.params.gas_per_batch / this.params.txs_per_batch;
        const rawCost = this.trendBasefee * 1e9 * gasCostPerTx / 1e18;
